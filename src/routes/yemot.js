...
// האישור של ימות במצב record דרך שלוחת ה-API (למרות מה שהתיעוד הכללי (הלא-ספציפי ל-API) מרמז), וגם
// בקשת סולמית (#) לא עזרה. אבל דרך כלי האבחון (GetIVR2Dir) ראינו את הטקסט המדויק שימות באמת משמיעה
// (הודעת מערכת M1553): "לאישור ההקלטה הקישו 1, לשמיעה הקישו 2, להקלטה מחדש 3" - כלומר יש להקיש
// בדיוק 1 כדי שההקלטה תאושר ותישמר בפועל. מנחים את המתקשר במפורש להקיש 1 בתום הדיבור, כדי שהוא ידע
