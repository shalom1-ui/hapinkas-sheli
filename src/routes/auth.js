...
function register(router) {
  ...
  router.post("/api/auth/signup", async (ctx) => {
    ...
    // תוקן (משוב אמיתי ממשתמש: "הטלפון צריך שיהיה חובה להרשמה") - טלפון היה עד עכשיו אופציונלי
    // בטופס ההרשמה באתר, מה שבפועל שבר את ההדדיות המתועדת בין הסיסמה לקוד ה-PIN הטלפוני: בלי מספר
    // טלפון שמור, אין שום דרך שהמשתמש הזה יזוהה אי-פעם בשיחה נכנסת (הזיהוי תמיד לפי Caller ID, ר'
    // findUserByPhone/routes/yemot.js) - כלומר הכיוון "הפוך" (סיסמת אתר עובדת גם כטלפון) לא היה
    // רלוונטי בפועל למי שנרשם בלי טלפון. עכשיו חובה, בדיוק כמו שם מלא/שם משתמש/סיסמה.
    if (!phone) {
      return json(ctx.res, 400, { error: "יש למלא מספר טלפון - הוא נדרש כדי שהזיהוי בשיחה נכנסת (וההתחברות בטלפון עם הסיסמה) יעבדו" });
    }
