...
function register(router) {
  ...
  router.post("/api/apartment/transactions", requireAuth(async (ctx) => {
    ...
    const info = db
      .prepare("INSERT INTO apartment_transactions (user_id, type, amount, category, note) VALUES (?, ?, ?, ?, ?)")
  ...
  router.put("/api/apartment/transactions/:id", requireAuth(async (ctx) => {
    ...
    db.prepare("UPDATE apartment_transactions SET type = ?, amount = ?, category = ?, note = ? WHERE id = ?")
