...
function sayAndRecord(text, path, fileName) {
  ...
  //
  // תוקן (אבחון בפועל מול קו אמיתי - ר' README): no_confirm_menu **חייב** להיות "yes", לא "no"!
  // התיעוד הרשמי של ימות אומר: "ברירת מחדל משמיע תפריט לאישור ההקלטה/הקלטה מחדש, ניתן להגדיר שמיד
  // בהקשה על סולמית ההקלטה תאושר" - כלומר "no_confirm_menu"="yes" הוא מה שמדלג על התפריט האינטראקטיבי
  // (שמבקש מהמתקשר להקיש כדי לאשר את ההקלטה או להקליט מחדש). כשהגדרנו את זה בטעות ל-"no" (שמשמעותו
  // בפועל "כן תציג תפריט אישור"), ימות המשיכה לחכות להקשת אישור שהמתקשר מעולם לא ידע שהוא צריך לתת
  // (השיחה שלנו לא מבקשת ולא מזכירה שום הקשה כזו) - ולכן ההקלטה נשארה תמיד "תלויה" בלי להישמר בפועל
  // לקובץ סופי, מה שהסביר את כשלי ה-404 העקביים בהורדה (הקובץ פשוט לא היה קיים - ר' GetIVR2Dir בכלי
  // האבחון ב-routes/yemot.js, שהראה תיקיית שלוחה ריקה לגמרי).
  // save_on_hangup="yes" - כדי שגם אם המתקשר מנתק מיד אחרי שאמר את השם, ההקלטה עדיין נשמרת.
  ...
  const ops = ["no", "record", path, fileName, "yes", "yes", "", "", "15"];
  return `read=t-${safe}=${VAL_NAME},${ops.join(",")}`;
}
